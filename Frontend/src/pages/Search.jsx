...
function Search() {
  ...
  // Get all songs from PlayerContext
  const {
    teluguSongs,
    hindiSongs,
    englishSongs,
  ...
  // Combine all songs into one array for global search
  const allSongs = [
    ...(teluguSongs || []),
    ...(hindiSongs || []),
    ...(englishSongs || []),
  ...
  // Support both old and new song object shapes
  const filteredTracks = allSongs.filter((track) => {
    // Try both 'title'/'artist' and 'name'/'artistNames' for compatibility
    const title = track.title || track.name || '';
    const artist = track.artist || track.artistNames || '';
    return (
  ...
  // Play preview (local audio) if available, else do nothing (for demo)
  const handleTrackClick = (track) => {
    if (currentTrack?.id === track.id) {
      ...
      setBgColor(colorMap[(track.id || track.name?.length || 0) % colorMap.length]);
    }
  };
  ...
  return (
    <div className={`p-8 min-h-screen transition-colors duration-500 ${bgColor}`}>
            ...
            {filteredTracks.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredTracks.map((track) => {
                  ...
                  const image = track.image || track.imageUrl || 'https://via.placeholder.com/80';
                  // Prefer src (local) or audioUrl (API)
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-light text-sm">{artist}{album ? ` • ${album}` : ''}</p>
                      </div>
                      {currentTrack && (currentTrack.id === track.id || currentTrack.name === track.name) && (
                        <motion.button
                          whileTap={{ scale: 0.9 }}
      ...
      {currentTrack && (
        <audio ref={audioRef}>
          <source src={currentTrack.src || currentTrack.audioUrl} type="audio/mpeg" />
